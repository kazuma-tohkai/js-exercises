...
test.describe("simple todo app", () => {
  ...
  test("complete todo", async ({ page, context }) => {
    ...
    await newTab.goto("/ch15.11-15/ex06");
    expect(await countToDos(newTab)).toBe(0);

    // sessionStorageはウィンドウ単位でスコープされているので、タブを開き直すとデータは消える
    await page.close(); // 現在のタブを閉じる
    const newPage = await context.newPage(); // 新しいタブを開く
    await newPage.goto("/ch15.11-15/ex06");
    expect(await countToDos(newPage)).toBe(0);
  });
});
